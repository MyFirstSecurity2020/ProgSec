...
# 1.radare2簡介
- Radare2擁有非常強大的功能，包括反組譯、調試、patch等等
...
- radare2 工具:radare2 有一些工具可供使用[本課程專注於radare2的技術]：
...
- STEP 1: (behavior of binary)先執行看看 程式執行的行為
  - file ./adder
  - strings ./adder 
```
.....
...
```
  - 用Visual Mode來看更清楚
    - [0x00400b1e]> s main　==> 先找(seek) main()函數 
      ...
      - [Seeking指令說明](https://book.rada.re/basic_commands/seeking.html)
...
- [0x00400860]> !python3
  - 開啟python3 shell ==> 進行簡單運算
  - 可以在radare2 session執行外部指令(要加!, 如!ls)
...
## 互動式使用方法:當進入到 Radare2 的互動式介面後，就可使用互動式命令進行操作。
